...
class Command(BaseCommand):
    help = 'Populate Item Equivalences from ESI static data export'
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for group in dogma_groups.values():
                if len(group) > 1:
                    ...
                    filtered = [tid for tid in group if tid in types and
                               types[tid].get('published', False) and
                               'marketGroupID' in types[tid]]
                    if len(filtered) > 1:
